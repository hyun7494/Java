 ...
 *  - Commit은 모든 작업 확정처리를, Rollback은 작업 전으로 돌아가기 위해 실패 처리
 */
public abstract class TransactionTest {
	public static void main(String[] args) {
		...
		String sql1 = "UPDATE `bank_account` SET `a_balance` = `a_balance` - 10000 WHERE `a_no`=? "; // 너무 길어서 빼놓은거
		String sql2 = "UPDATE `bank_account` SET `a_balance` = `a_balance` + 10000 WHERE `a_no`=? "; // 너무 길어서 빼놓은거
